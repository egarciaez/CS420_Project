...
class DroneNavigation:
    ...
    def create_background(self):
        ...
        # Horizontal road
        self.canvas.create_rectangle(
            0, self.horizontal_road_top_y, self.canvas_width, self.horizontal_road_bottom_y,
            fill="#5B5F62", outline=""
        )
        ...
        self.canvas.create_rectangle(
            v_left, 0, v_right, self.canvas_height,
            fill="#5B5F62", outline=""
        )
        ...
        while x < self.canvas_width:
            self.canvas.create_rectangle(
                x, stripe_y - 2, x + stripe_width, stripe_y + 2,
                fill="#E6D35C", outline=""
            )
        ...
        while y < self.canvas_height:
            self.canvas.create_rectangle(
                stripe_x - 2, y, stripe_x + 2, y + stripe_width,
                fill="#E6D35C", outline=""
            )
        ...
        for sx, sy in sign_positions:
            ...
            self.canvas.create_polygon(
                ...
                sx + 10, sy, sx + 7, sy + 7,
                sx - 7, sy + 7, sx - 10, sy,
                fill="#C62828", outline="#8D1D1D"
            )
            self.canvas.create_text(
                sx, sy, text="STOP", fill="white",
                font=("Arial", 5, "bold")
            )
    ...
    def create_drone(self):
        ...
        self.canvas.create_rectangle(
            x1, y1, x2, y2,
            fill="#2F7FBF", outline="#1D4B75", width=2, tags=self.drone_tag
        )
        self.canvas.create_line(
            x1 - arm, start_y, x2 + arm, start_y,
            fill="#1D4B75", width=2, tags=self.drone_tag
        )
        self.canvas.create_line(
            start_x, y1 - arm, start_x, y2 + arm,
            fill="#1D4B75", width=2, tags=self.drone_tag
        )
        ...
        for rx, ry in [
            ...
            self.canvas.create_oval(
                ...
                fill="#BFC7CF", outline="#7D8893", tags=self.drone_tag
            )

        self.canvas.create_oval(
            start_x - 3, start_y - 3, start_x + 3, start_y + 3,
            fill="#FFFFFF", outline="", tags=self.drone_tag
        )
    ...
    def create_obstacles(self):
        ...
        for tree_center_x, tree_center_y in tree_positions:
            ...
            self.canvas.create_rectangle(
                tree_center_x - trunk_width / 2, trunk_top_y,
                tree_center_x + trunk_width / 2, trunk_bottom_y,
                fill="#7C4A27", outline="#5E351C"
            )
            self.canvas.create_oval(
                tree_center_x - canopy_radius, trunk_top_y - 20,
                tree_center_x + canopy_radius, trunk_top_y + 14,
                fill="#2E8B57", outline="#206642"
            )
        ...
        for building_center_x, building_center_y, building_width, building_height, is_skyscraper in building_specs:
            ...
            self.canvas.create_rectangle(
                ...
                outline="#5F6870", width=2
            )
            roof_h = 10 if not is_skyscraper else 6
            self.canvas.create_rectangle(
                x1 - 3, y1 - roof_h, x2 + 3, y1,
                fill="#6E747C", outline="#4B5056"
            )
    ...
    def point_inside_rect(self, point_x, point_y, rect, padding=0):
        x1, y1, x2, y2 = rect
        return (
            x1 - padding <= point_x <= x2 + padding
            and y1 - padding <= point_y <= y2 + padding
        )
    ...
    def has_line_of_sight_to_crash(self):
        ...
        return self.get_blocking_structure(
            current_x, current_y, self.crash_site_x, self.crash_site_y
        ) is None
    ...
    def is_on_road(self, x, y, padding=0):
        ...
        on_vertical = (
            self.vertical_road_left_x + padding <= x <= self.vertical_road_right_x - padding
        )
    ...
    def move_toward(self, target_x, target_y, speed=4.5):
        ...
        # If a detour is active, keep following it before retrying direct travel.
        ...
        if self.detour_waypoint is None:
            ...
            if blocker:
                self.detour_waypoint = self.choose_detour_waypoint(
                    blocker, goal_x, goal_y, current_x, current_y
                )
    ...
    def move(self):
        ...
        # Two parts: search (scan) first, then final approach to crash site.
...
if __name__ == "__main__":
    ...
    def run_simulation():
        # Stop old mission loop first so old and new animation loops do not overlap.
        # Then clear old sprites/markers before creating a fresh mission.
